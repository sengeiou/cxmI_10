...
class BasicDialog: SpringView {
    ...
    open func createCloseBtn() {
        ...
        closeBtn.setBackgroundImage(UIImage.init(named: "关闭"), for: UIControl.State.normal)
        closeBtn.sizeToFit()
        addSubview(closeBtn)
        ...
        closeBtn.addTarget(self, action: #selector(hide), for: UIControl.Event.touchUpInside)
    }

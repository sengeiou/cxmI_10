//
//  HXGuideMaskView.swift
//  tips
//
//  Created by 笑 on 2018/12/12.
//  Copyright © 2018 笑. All rights reserved.
//
...
class HXGuideMaskView: UIView {
    ...
    private var showDatas : [HXGuideInfoModel] = []
    ...
    func showMask(data : [HXGuideInfoModel]) {
        self.showDatas = data
        self.currentIndex = 0
    ...
    private func showItem(data : HXGuideInfoModel) {
    ...
    ///
    /// - Parameter data: 蒙版数据源
    private func configItemsFrame(data : HXGuideInfoModel) {
        ...
        if let desc = data.text as NSString? {
            textSize = desc.boundingRect(with: CGSize.init(width: maxWidth, height: CGFloat.greatestFiniteMagnitude), options: [NSStringDrawingOptions.usesLineFragmentOrigin,NSStringDrawingOptions.usesFontLeading], attributes: [NSAttributedString.Key.font : data.font], context: nil).size
        }
        ...
        case .rightTop:
            /// 右上
            arrowRect = CGRect.init(x: self.fetchVisualFrame().midX - imgSize.width * 0.8, y: self.fetchVisualFrame().maxY + data.space, width: imgSize.width, height: imgSize.height)
            var x : CGFloat = 0
            if textSize.width < self.fetchVisualFrame().width {
            ...
            textRect = CGRect.init(x: x, y: arrowRect.maxY, width: textSize.width, height: textSize.height)
            buttonRect = CGRect(x: UIScreen.main.bounds.size.width / 2 - 62, y: textRect.maxY + 40, width: 124, height: 34)
            break
        case .leftBottom:
            /// 左下
            transform = CGAffineTransform.init(scaleX: -1, y: -1)
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.confirmButton.frame = buttonRect
        }
    }
    ...
    private lazy var confirmButton : UIButton = {
        ...
        but.addTarget(self, action: #selector(dismisView), for: .touchUpInside)
        return but
    }()
...
/*👋 每一个提示的属性 👋*/
class HXGuideInfoModel: NSObject {
    /*👋 每个item的文字 👋*/
    var text : String?
    /*👋 每个item的文字颜色 👋*/
    ...
    var insetEdge = UIEdgeInsets(top: -8, left: -8, bottom: -8, right: -8)
